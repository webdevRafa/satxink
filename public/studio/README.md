...
Capable phones and desktops automatically play the path when visible. Reduced-motion/data-saving settings and lower-spec devices still default to static images. Optional 3D can be paused or swapped to a poster, and falls back on load/WebGL/performance failure. Leaving the viewport or hiding the tab aborts pending loading and disposes the canvas, model, renderer, and WebGL context. Re-entry recreates the animation automatically unless the user chose Static or a permanent failure occurred; Pause remains respected. Playback controls sit outside the image so they cannot obscure branding.
