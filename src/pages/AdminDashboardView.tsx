        ...
        {links.map((link) => {
          ...
          return (
            ...
            >
              ...
              {typeof counts[link.key] === "number" && (counts[link.key] || 0) > 0 && (
                <span
          ...
          <div className="flex min-h-full items-start justify-center p-4 text-center sm:p-6">
            ...
            >
              <Dialog.Panel className="relative w-full max-w-2xl transform overflow-hidden rounded-lg bg-[#111] p-6 text-left align-middle shadow-xl transition-all">
                ...
                <div className="mt-4 max-h-[70vh] overflow-y-auto border-t border-white/10 pt-4">
                  {item && (
...
const ArtistsTable: React.FC<TableProps<ArtistRecord>> = ({ data, onSelect }) => {
  return (
        ...
        <div className="min-w-[600px] divide-y divide-white/10">
          ...
          {data.map((artist) => (
            ...
            >
              ...
              <span>
                {artist.createdAt && typeof artist.createdAt.toDate === 'function'
                  ? new Date(artist.createdAt.toDate()).toLocaleDateString()
                  : artist.createdAt?.seconds
                  ? new Date(artist.createdAt.seconds * 1000).toLocaleDateString()
                  : '-'}
              </span>
            </button>
          ))}
...
const RequestsTable: React.FC<TableProps<GenericRecord>> = ({ data, onSelect }) => {
  return (
    <section className="space-y-4">
...
const OffersTable: React.FC<TableProps<GenericRecord>> = ({ data, onSelect }) => {
  return (
    <section className="space-y-4">
...
const BookingsTable: React.FC<TableProps<GenericRecord>> = ({ data, onSelect }) => {
  return (
    <section className="space-y-4">
...
const SessionsTable: React.FC<TableProps<GenericRecord>> = ({ data, onSelect }) => {
  return (
    <section className="space-y-4">
        ...
        <div className="min-w-[800px] divide-y divide-white/10">
          ...
          {data.map((session) => (
            ...
            >
              ...
              <span>
                {session.scheduledAt && typeof session.scheduledAt.toDate === 'function'
...
const AdminDashboardView: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<GenericRecord | ArtistRecord | null>(null);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-[#121212] via-[#0f0f0f] to-[#121212] pt-20 text-white md:flex-row">
      ...
      <main className="flex-1 p-6 space-y-8">
        {activeView === 'artists' && (
          <ArtistsTable data={artists} onSelect={(item) => setSelectedItem(item)} />
        )}
        {activeView === 'requests' && (
          <RequestsTable data={requests} onSelect={(item) => setSelectedItem(item)} />
        )}
        {activeView === 'offers' && (
          <OffersTable data={offers} onSelect={(item) => setSelectedItem(item)} />
        )}
        {activeView === 'bookings' && (
          <BookingsTable data={bookings} onSelect={(item) => setSelectedItem(item)} />
        )}
        {activeView === 'sessions' && (
          <SessionsTable data={sessions} onSelect={(item) => setSelectedItem(item)} />
        )}
      </main>
